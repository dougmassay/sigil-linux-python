...
if [[ -x "${python}" ]]; then
    ...
    cd "${source_dir}"
    ./configure ${PYTHON_CONFIG} "--enable-shared" "--with-ensurepip=install" "--prefix=/${prefix}" LDFLAGS="${LDFLAGS} -Wl,-rpath='"'$$ORIGIN'"/../../lib'"
...
if [[ ! -z "${PIP_REQUIREMENTS}" ]]; then
    export LD_LIBRARY_PATH="${APPDIR}/${prefix}/lib:$LD_LIBRARY_PATH"
...
# Move shared libpython to AppDir/usr/lib
cd "$APPDIR/${prefix}/lib"
set +e
py_libs=$(ls "libpython"?"."?".so"* "libpython"?".so"  2>/dev/null)
set -e
for py_lib in $py_libs
do
    cp -P "${py_lib}" "${APPDIR}/usr/lib"
    if [[ ! -L ${py_lib} ]]; then
        "${patchelf}" --set-rpath '$ORIGIN' "${APPDIR}/usr/lib/${py_lib}"
    fi
    rm "${py_lib}"
done
...
if [[ ! -d "${APPDIR}/${prefix}/share/tcltk" ]]; then
    ...
    tclpath="$(ls -d ${TCL_DIR}/lib/tcl8* | tail -1)"
    tkpath="$(ls -d ${TK_DIR}/lib/tk8* | tail -1)"
    echo "${tclpath}" "${tkpath}"
